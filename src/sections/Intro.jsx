...
const MIN_LEN = 25; // px   (shortest gradient bar)
const MAX_LEN = 40; // px   (longest  gradient bar)
const MAX_NEG_DELAY = 3; // secs (random negative offset)

/* 50 curved guide-paths ------------------------- */
const streakPaths = [
...
const StreakBackground = memo(() => {
  const active = useMemo(
    () =>
      streakPaths
        .map((d, i) => ({ d, i }))
        .filter(() => Math.random() < ACTIVE_FRACTION),
    []
  );
  ...
  return (
    <>
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(rgba(255,255,255,0.18) 0.7px, transparent 1px)",
      ...
      >
        ...
        {active.map(({ i }) => {
          ...
          const len = Math.random() * (MAX_LEN - MIN_LEN) + MIN_LEN;
          const negDelay = (-Math.random() * MAX_NEG_DELAY).toFixed(2) + "s";
          return (
            <rect
              key={`star-${i}`}
              width={len}
        ...
        <defs>
          {streakPaths.map((_, i) => (
            ...
            >
              ...
              <stop offset="80%" stopColor="#6344F5" stopOpacity="1" />
        ...
        <style>{`
       @keyframes dash {
          to { stroke-dashoffset: var(--dash-total-neg); }
        }
      `}</style>
      </svg>
    </>
...
export default function Intro() {
  ...
  return (
    <section
      ...
      style={{ backgroundColor: "#000", paddingTop: "35vh" }}
    >
      <StreakBackground />

      <h1 className="relative z-10 text-center text-3xl font-bold sm:text-5xl text-white">
        {prefix && <>{prefix} </>}
